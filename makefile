...
$(OSAX_SRC): $(OSAX_PATH)/loader.m $(OSAX_PATH)/payload.m
	clang \
		...
		-isystem @CLANG_LIB@/lib/clang/*/include \
		-isystem @CUPS_DEV@/include \
		-F$(SDKROOT)/System/Library/Frameworks \
		-L$(SDKROOT)/usr/lib \
		$(OSAX_PATH)/payload.m \
		-shared -fPIC -O3 -mmacosx-version-min=11.0 \
		-arch @ARCH_SA@ \
		-o $(OSAX_PATH)/payload $(FRAMEWORK_PATH) \
		-framework SkyLight \
